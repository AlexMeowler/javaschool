...
import org.retal.logiweb.service.logic.interfaces.OrderServices;
...
/**
 * Implementation of {@link OrderServices} interface.
 * 
...
public class OrderService implements OrderServices {
  ...
  private final CompletedOrderInfoDAO completedOrderInfoDAO;
  ...
  public OrderService(CityDAO cityDAO, CityDistanceDAO cityDistanceDAO, OrderDAO orderDAO,
      CompletedOrderInfoDAO completedOrderInfoDAO, UserDAO userDAO,
      OrderRouteProgressionDAO orderRouteProgressionDAO, CarService carService,
  ...
  public List<Order> getAllActiveOrders() {
    return getAllOrders().stream().filter(o -> !o.getIsCompleted()).collect(Collectors.toList());
  }
  ...
  public boolean checkOrderForCompletion(Order order) {
    ...
    if (isCompleted) {
      ...
      for (UserInfo driverInfo : order.getDriverInfo()) {
        builder.append(String.format("%s %s (%d)" + CompletedOrderInfo.DRIVERS_DELIMETER,
            driverInfo.getName(), driverInfo.getSurname(), driverInfo.getUser().getId()));
      ...
      CompletedOrderInfo additionalInfo =
          new CompletedOrderInfo(order.getCar().getRegistrationId(), drivers, order);
      completedOrderInfoDAO.add(additionalInfo);
  ...
  public Object[] findAppropriateCarAndDriversAndCalculatePath(List<RoutePoint> list,
    ...
    String debugStatus = builder.toString();
      ...
      if (rp.getIsLoading()) {
        ...
        if (loadIndex == -1 || unloadIndex == -1) {
          ...
          for (int path : annealing) {
            builder.append(rpCities.get(path).getCurrentCity() + PATH_DELIMETER);
          }
   ...
   * {@linkplain #createOrderFromRoutePoints(RoutePointListWrapper, BindingResult) here}
   * 
    ...
    for (int i = 0; i < n; i++) {
      for (int j = i + 1; j < n; j++) {
        ...
        if (bigMatrix[k][m] != Integer.MAX_VALUE / 2) {
          ...
          String path = builder.toString();
          path = path.substring(0, path.length() - PATH_DELIMETER.length());
          log.debug(path);
        }
      }
    }
